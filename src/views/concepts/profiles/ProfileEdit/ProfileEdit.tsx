...
import { apiUpdateProfile } from '@/services/ProfilesService'
...
const ProfileEdit = () => {
    ...
    const handleFormSubmit = async (values: ProfileFormSchema) => {
        setIsSubmiting(true)
        try {
            const response = apiUpdateProfile(values);
            setIsSubmiting(false)

            if ((await response).status === 200) {
                toast.push(
                    <Notification type="success">پروفایل ویرایش شد!</Notification>,
    ...
    return (
        <>
            ...
            {!isLoading && data && (
                <>
                    <ProfileForm
                        defaultValues={getDefaultValues() as ProfileFormSchema}
                        newProfile={false}
                    ...
                    >
                        <p>
                            آیا مطمئنید که می خواهید این پروفایل را حذف کنید؟ این
