...
import { apiGetProfile } from '@/services/ProfilesService'
...
import type { FormSectionBaseProps } from '../ProfileForm/types'
...
const ProfileDetails = ({ control, errors }: OverviewSectionProps) => {
    const { id } = useParams()

    const navigate = useNavigate()

    const { data, isLoading } = useSWR(
        [`/api/profile{id}`, { id: id as string }],
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        ([_, params]) => apiGetProfile<Profile>(id),
        {
        ...
        <>
            {!isLoading && !data && (
                <div className="h-full flex flex-col items-center justify-center">
                    ...
                    <h3 className="mt-8">پروفایل پیدا نشد!</h3>
                </div>
            )}
            {!isLoading && data && (
                <>
                            ...
                            <div className="gap-4 flex flex-col flex-auto">
                                <Card>
                                    ...
                                    <div className="grid md:grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            <span className="font-semibold">برنچ</span>
                                            <p className="heading-text font-bold">{data?.branch}</p>
                                        </div>

                                        <div>
                                            <span className="font-semibold">برند</span>
                                            <p className="heading-text font-bold">{data?.brandTitle}</p>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <p className="heading-text font-bold">{data?.productTitle}</p>
                                        </div>
                                        <div>
