...
import { apiCreateCustomerPhone } from '@/services/CustomerPhonesService'
...
const CustomerPhoneEdit = () => {
    ...
    const handleFormSubmit = async (values: CustomerPhoneFormSchema) => {
        setIsSubmiting(true)
        try {
            const response = apiCreateCustomerPhone(values);
            setIsSubmiting(false)

            if ((await response).status === 200) {
                toast.push(
                    <Notification type="success">تلفن مشتری ایجاد شد!</Notification>,
    ...
    const handleConfirmDiscard = () => {
        ...
        toast.push(
            <Notification type="success">تلفن مشتری کنار گذاشته شد!</Notification>,
            { placement: 'top-center' },
        )
        navigate('/concepts/customerPhones/customerPhone-list')
    }
