...
import { apiCreateCustomerProfile } from '@/services/CustomerProfilesService'
...
const CustomerProfileEdit = () => {
    ...
    const handleFormSubmit = async (values: CustomerProfileFormSchema) => {
        setIsSubmiting(true)
        try {
            const response = apiCreateCustomerProfile(values);
            setIsSubmiting(false)

            if ((await response).status === 200) {
                toast.push(
                    <Notification type="success">پروفایل مشتری ایجاد شد!</Notification>,
    ...
    const handleConfirmDiscard = () => {
        ...
        toast.push(
            <Notification type="success">پروفایل مشتری کنار گذاشته شد!</Notification>,
            { placement: 'top-center' },
        )
        navigate('/concepts/customerProfiles/customerProfile-list')
    }
