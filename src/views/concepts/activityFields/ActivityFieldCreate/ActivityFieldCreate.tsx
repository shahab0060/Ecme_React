...
import { apiCreateActivityField } from '@/services/ActivityFieldsService'
...
const ActivityFieldEdit = () => {
    ...
    const handleFormSubmit = async (values: ActivityFieldFormSchema) => {
        setIsSubmiting(true)
        try {
            const response = apiCreateActivityField(values);
            setIsSubmiting(false)

            if ((await response).status === 200) {
                toast.push(
                    <Notification type="success">حوزه کاری ایجاد شد!</Notification>,
    ...
    const handleConfirmDiscard = () => {
        ...
        toast.push(
            <Notification type="success">حوزه کاری کنار گذاشته شد!</Notification>,
            { placement: 'top-center' },
        )
        navigate('/concepts/activityFields/activityField-list')
    }
