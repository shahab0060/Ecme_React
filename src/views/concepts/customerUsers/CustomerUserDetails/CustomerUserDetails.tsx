...
import { apiGetCustomerUser } from '@/services/CustomerUsersService'
...
import type { CustomerUser } from '../CustomerUserList/types'
import Input from '@/components/ui/Input'
import { FormItem } from '@/components/ui/Form'
import type { FormSectionBaseProps } from '../CustomerUserForm/types'
...
const CustomerUserDetails = ({ control, errors }: OverviewSectionProps) => {
    const { id } = useParams()

    const navigate = useNavigate()

    const { data, isLoading } = useSWR(
        [`/api/customerUser{id}`, { id: id as string }],
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        ([_, params]) => apiGetCustomerUser<CustomerUser>(id),
        {
        ...
        <>
            {!isLoading && !data && (
                <div className="h-full flex flex-col items-center justify-center">
                    ...
                    <h3 className="mt-8">کاربر مشتری پیدا نشد!</h3>
                </div>
            )}
            {!isLoading && data && (
                <>
