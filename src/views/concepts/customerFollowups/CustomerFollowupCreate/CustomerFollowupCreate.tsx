...
import { apiCreateCustomerFollowup } from '@/services/CustomerFollowupsService'
...
const CustomerFollowupEdit = () => {
    ...
    const handleFormSubmit = async (values: CustomerFollowupFormSchema) => {
        setIsSubmiting(true)
        try {
            const response = apiCreateCustomerFollowup(values);
            setIsSubmiting(false)

            if ((await response).status === 200) {
                toast.push(
                    <Notification type="success">پیگیری مشتری ایجاد شد!</Notification>,
    ...
    const handleConfirmDiscard = () => {
        ...
        toast.push(
            <Notification type="success">پیگیری مشتری کنار گذاشته شد!</Notification>,
            { placement: 'top-center' },
        )
        navigate('/concepts/customerFollowups/customerFollowup-list')
    }
